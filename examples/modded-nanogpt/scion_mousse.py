...
class MousseScion(torch.optim.Optimizer):
    ...
    Every `eig_update_freq` steps:
        (Λ_L, Q_L) = eigh(L̂_t),   (Λ_R, Q_R) = eigh(R̂_t)      [eigendecompositions]

    Whitening:
        M̃      =  Q_L^T  m_t  Q_R                                [rotate]
        M̃_{ij} /= λ_i^(L,α) · λ_j^(R,α)                        [scale]

    LMO in whitened space:
        u      =  lmo(M̃)
        n*     =  ‖u‖_F                                          [graft reference norm]
        ρ      =  ⟨u, M̃⟩                                        [dual norm / duality gap]

    Unwhitening:
        u_{ij} /= λ_i^(L,α) · λ_j^(R,α)                        [inverse scale]
        u      =  Q_L  u  Q_R^T                                  [inverse rotate]

    Grafting:
        u  ←  (n* / ‖u‖_F) · u                                  [restore LMO norm]

    Frank-Wolfe parameter update:
    ...
    Args:
        ...
        use_dual_norm (bool):    Multiply update by ρ = ⟨lmo(M̃), M̃⟩ (duality gap).
        ...
        apply_grafting (bool):   After unwhitening rescale u to match the pre-unwhiten LMO norm.
                                 Prevents eigenvalue round-trip from inflating/deflating the step
                                 size (default: True).
    """
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # ρ = ⟨lmo(M̃), M̃⟩  computed in the whitened space so it is
