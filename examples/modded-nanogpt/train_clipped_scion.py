...
class UnconstrainedClippedScion(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-3, momentum=1.0, norm: str='Auto', norm_kwargs: dict=None, scale=1.0, rho=600):
        if lr < 0.0:
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                lmo_g = norm_backend.lmo(g) * scale
                self.dual_norm += (lmo_g * g).sum()
                
                state["update"] = lmo_g
        
        dist.all_reduce(self.dual_norm, op=dist.ReduceOp.SUM)
        
        for group in self.param_groups:
            lr = group['lr']
            rho = group['rho']
            eta = min(1, self.dual_norm.item() / rho)
            self.effective_lrs[group['norm']] = eta * lr
            for p in group['params']:
                update = eta * self.state[p]["update"]
...
class ClippedScion(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-3, momentum=1.0, norm: str='Auto', norm_kwargs: dict=None, scale=1.0, rho=600):
        if lr < 0.0:
            raise ValueError(f"Invalid learning rate: {lr}")
        if momentum < 0.0:
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                lmo_g = - norm_backend.lmo(g) * scale
                self.ucg_bound += ((x - lmo_g) * g).sum()
                
                state["update"] = x - lmo_g

        dist.all_reduce(self.ucg_bound, op=dist.ReduceOp.SUM)

        for group in self.param_groups:
            lr = group['lr']
            rho = group['rho']
            eta = min(1, self.ucg_bound.item()/rho)
            self.effective_lrs[group['norm']] = eta * lr
            for p in group['params']:
                update = eta * self.state[p]["update"]
                p.data.add_(update, alpha=-lr) 
...
OPTIMIZER = UnconstrainedClippedScion if args.unconstrained else ClippedScion
optimizer1 = OPTIMIZER(optim_groups, lr=args.learning_rate, momentum=args.momentum, rho=args.threshold)
