...
class WarmupScion(torch.optim.Optimizer):
...
def compute_LR_inv(L, R, order=8, eps_stab=1e-4):
    ...
    L_d = L.double() + eps_stab * torch.eye(dim_L, device=L.device, dtype=torch.float64)
    R_d = R.double() + eps_stab * torch.eye(dim_R, device=R.device, dtype=torch.float64)
