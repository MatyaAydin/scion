...
Version: https://github.com/KellerJordan/cifar10-airbench/blob/1da61ae58ee9c112e7f166fac3b97d245aa72942/airbench94_muon.py
"""
...
def zeroth_power_via_randSVD(G):
    U, S, V = torch.svd_lowrank(G.float(), q=6, niter=2)
    return (U @ V.T).type(G.dtype)
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if hyp['opt']['svd_backend'] == 'newton':
                    update = zeroth_power_via_newton(g.reshape(len(g), -1)).view(g.shape) # whiten the update
                elif hyp['opt']['svd_backend'] == 'randSVD':
                    update = zeroth_power_via_randSVD(g.reshape(len(g), -1)).view(g.shape)
                else:
                    update = zeroth_power_via_svd(g.reshape(len(g), -1)).view(g.shape)
